...
export function Admin() {
 ...
 const [comments, setComments] = useState<CadernoComment[]>([]);
 ...
 async function loadComments() {
   ...
   try {
     const { data, error } = await supabase
       ...
       .select('id, post_id, user_id, body, approved, created_at')
     ...
     setComments((data ?? []) as CadernoComment[]);
   } catch (error) {
     console.error(error);
     setCommentErrorMessage(
       error instanceof Error
 ...
 async function handleCommentApproval(comment: CadernoComment) {
   ...
   try {
     ...
     setCommentMessage(
       comment.approved
         ? 'Comentário removido da publicação.'
 ...
 async function handleCommentDelete(comment: CadernoComment) {
   const confirmed = window.confirm(
     'Remover este comentário definitivamente?'
   );
   ...
   try {
     ...
     setCommentMessage('Comentário removido.');
     await loadComments();
   } catch (error) {
     console.error(error);
     setCommentErrorMessage(
       ...
       <section style={{ marginTop: 72 }}>
         ...
         >
           <div>
             ...
             >
               Aprove, retire da publicação ou exclua mensagens enviadas no Caderno.
             </p>
           </div>

           <span
             style={{
               border: '1px solid rgba(179,151,114,.35)',
               color: '#66705f',
               fontSize: 12,
               padding: '9px 12px',
             }}
         ...
         {commentsLoading ? (
           ...
           >
             Ainda não há comentários enviados.
           </div>
         ) : (
           <div
             style={{
           ...
           >
             {comments.map((comment) => {
               const isWorking = commentActionId === comment.id;
               ...
               return (
                 ...
                 >
                   ...
                   >
                     Enviado em {formatCommentDate(comment.created_at)}
                   </small>
                   ...
                   >
                     <button
                       ...
                       disabled={isWorking}
                       onClick={() => {
                         void handleCommentApproval(comment);
                       }}
                       style={{
                         border: 0,
                         background: '#26321f',
